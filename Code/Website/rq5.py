...
# ── Paths ─────────────────────────────────────────────────────────────────────
BASE_DIR         = "../../data"
REGISTRATION_CSV = os.path.join(BASE_DIR, "RegistrationData", "kiel_registration_data.csv")
AIR_QUALITY_CSV  = os.path.join(BASE_DIR, "AirQuality", "air_quality_kiel.csv")
STATIONS_CSV     = os.path.join(BASE_DIR, "MergeData", "holy_file.csv")
...
@st.cache_data(show_spinner="Loading station traffic data …")
...
try:
    df = load_registrations(REGISTRATION_CSV)
except FileNotFoundError:
    st.error(f"CSV not found: {REGISTRATION_CSV}")
    st.stop()

# Chart 1
is_stacked = st.radio("Bar mode", ["Stacked", "Grouped"], horizontal=True) == "Stacked"

fig1, totals = build_registration_chart(df, is_stacked)
st.plotly_chart(fig1, use_container_width=True)

with st.expander("Raw data table"):
    table = df.select(["Year"] + list(FUEL_COLS.keys())).rename(FUEL_COLS)
...
try:
    no2_monthly = load_no2_monthly(AIR_QUALITY_CSV)
    vehicles    = load_vehicles_by_year(STATIONS_CSV)
    fig2, summary, common_years = build_bev_no2_chart(df, totals, no2_monthly, vehicles)
